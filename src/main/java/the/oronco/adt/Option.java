...
/**
 * Similar to {@link java.util.Optional} in that it describes a value that can either be there or not. But better as it can be used in the new switch
...
public sealed interface Option<T> extends Rusty<Optional<T>>, Try<T, Option<Infallible>>, Streamable<T>, Serializable {
    ...
    /**
     * Creates a {@link java.util.stream.Stream} from the {@code Option<T>} that contains the value of {@code Some<T>} and is empty otherwise.
     *
     ...
     * @throws the.oronco.adt.exceptions.UnwrappedBadValueException with the given Error message when the {@code Option<T>} is {@code None<T>}
     */
    default @NotNull T unwrap(String errorMessage) throws @NotNull UnwrappedBadValueException {
        return switch (this) {
            case Some<T>(T value) -> value;
            case None<T> ignored -> throw new UnwrappedBadValueException(errorMessage);
        };
    }
     ...
     * @throws E with the given Exception when the {@code Option<T>} is {@code None<T>}
     */
    default @NotNull <E extends Exception> T unwrapElse(@NotNull @NonNull Supplier<@NotNull E> exceptionSupplier) throws @NotNull E {
        return switch (this) {
            case Some<T>(T value) -> value;
    ...
    /**
     * Returns the contained {@code Some<T>} value and throws otherwise. (Similar to {@link Option#unwrap(String)} but without a custom
     * error message)
     * <p>
     ...
     *
     * @return the value if the {@code Option<T>} is {@code Some<T>}
     * @throws the.oronco.adt.exceptions.UnwrappedBadValueException when the {@code Option<T>} is {@code None<T>}
     */
    @Contract(pure = true)
    default @NotNull T unwrap() throws @NotNull UnwrappedBadValueException {
        return switch (this) {
            case Some<T>(T value) -> value;
            case None<T> ignored -> throw new UnwrappedBadValueException("Option was unwrapped but it had no value!");
        };
    }

    /**
     * Returns the contained {@code Some<T>} value and a default value otherwise.<p> Arguments passed to {@link Option#unwrapOr(T)} are
     * eagerly evaluated; if you are passing the result of a function call, it is recommended to use
     * {@link Option#unwrapOrElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
     ...
     * @return a new {@code Option<R>} with the converted value
     */
    default <R, X extends Exception> @NotNull Option<R> safeMap(
            @NotNull @NonNull ThrowingFunction<? super @NotNull T, ? extends @NotNull R, ? extends @NotNull X> f) throws WrongKindOfExceptionError {
        return switch (this) {
            case Some<T>(T value) -> switch (f.apply(value)
                                              .ok()) {
                case Some<? extends R>(R val) -> some(val);
                case None<? extends R> none -> none.as();
            };
            case None<T> ignored -> none();
        };
    }
    ...
    /**
     * Calls the provided {@code Consumer<T>} with the contained value (if {@code Some<T>}) and returns itself. This allows for chaining multiple
     * consumers that all need the value e.g.:
     * <pre>
    ...
    /**
     * Returns the provided default result (if {@code None<T>}), or applies a {@code Fuction<T,R>} to the contained value (if {@code Some}).
     * <p>
    ...
    default <R> @NotNull R mapOr(
            @NotNull @NonNull Function<? super @NotNull T, ? extends @NotNull R> f, @NotNull @NonNull R defaultValue) {
        return switch (this) {
     ...
     */
    default <R> @NotNull R mapOrElse(
            @NotNull @NonNull Function<? super @NotNull T, ? extends @NotNull R> f, @NotNull @NonNull Supplier<? extends @NotNull R> d) {
        return switch (this) {
            case Some<T>(T value) -> f.apply(value);
    ...
    /**
     * Transforms the {@code Option<T>} into a {@code Result<T, E>}, mapping {@code Some(v)} to {@code Ok(v)} and {@code None} to {@code Err(err)}.
     * <p>
     * Arguments passed to {@link Option#okOr(Object)} are eagerly evaluated; if you are passing the result of a function call, it is
     * recommended to use {@link Option#okOrElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
    ...
    /**
     * Returns {@code None} if the {@code Option<T>} is {@code None}, otherwise calls the {@code Function<T,Option<U>>} with the wrapped value and
     * returns the result.
     * <p>
    ...
    /**
     * Returns the {@code Option<T>} if it contains a value, otherwise returns {@code other} as an {@code Option<T>}. This means if other is
     * {@code null} the returned option is {@code None<T>}.
     * <p>
     * Arguments passed to or are eagerly evaluated; if you are passing the result of a function call, it is recommended to use
     ...
     *
     * @param other a {@code Supplier} that provides the {@code other} {@code Option<T>} in case that {@code this} is {@code None<T>}
     * @return the {@code Option<T>} if it contains a value, otherwise calls the {@code Supplier} and returns the result in an {@code Option<T>}
     */
    @Contract("_ -> new")
