...
import the.oronco.adt.exceptions.OptionEmptyException;
...
/**
 * Similar to {@link java.util.Optional} in that it describes a value that can either be there or not. But better as it can be
 * used in the new switch pattern matching. It is inspired by the rust algebraic type of <a
...
public sealed interface Option<T> extends Rusty<Optional<T>>, Try<T, Option<Infallible>>, Streamable<T>, Serializable {
    ...
    @Unmodifiable record Some<T>(@NotNull @NonNull T value) implements Option<@NotNull T> {}
    ...
    /**
     * Creates a {@link java.util.stream.Stream} from the {@code Option<T>} that contains the value of {@code Some<T>} and is
     * empty otherwise.
     *
     ...
     *
     * @throws OptionEmptyException with the given Error message when the {@code Option<T>} is {@code None<T>}
     */
    default @NotNull T unwrap(String errorMessage) throws @NotNull OptionEmptyException {
        return switch (this) {
            case Some<T>(T value) -> value;
            case None<T> ignored -> throw new OptionEmptyException(errorMessage);
        };
    }

    /**
     * @param error the Exception that is thrown when the {@code Option<T>} is {@code None<T>}
     *
     * @return the value if the {@code Option<T>} is {@code Some<T>}
     *
     * @throws E with the given Exception when the {@code Option<T>} is {@code None<T>}
     */
    default @NotNull <E extends Exception> T unwrap(@NonNull E error) throws @NotNull E {
        return switch (this) {
            case Some<T>(T value) -> value;
            case None<T> ignored -> throw error;
        };
    }

    /**
     * @param exceptionSupplier supplier that returns the Exception that is thrown when the {@code Option<T>} is {@code None<T>}
     *
     * @return the value if the {@code Option<T>} is {@code Some<T>}
     *
     * @throws E with the given Exception when the {@code Option<T>} is {@code None<T>}
    ...
    /**
     * Returns the contained {@code Some<T>} value and throws otherwise. (Similar to {@link Option#unwrap(String)} but without a
     ...
     * {@link Option#unwrapOr(Object)} or {@link Option#unwrapOrElse(java.util.function.Supplier)} instead.
     *
     * @return the value if the {@code Option<T>} is {@code Some<T>}
     *
     * @throws OptionEmptyException when the {@code Option<T>} is {@code None<T>}
     */
    @Contract(pure = true)
    default @NotNull T unwrap() throws @NotNull OptionEmptyException {
        return switch (this) {
            case Some<T>(T value) -> value;
            case None<T> ignored -> throw new OptionEmptyException("Option was unwrapped but it had no value!");
        };
    }

    /**
     * Returns the contained {@code Some<T>} value and a default value otherwise.<p> Arguments passed to
     * {@link Option#unwrapOr(T)} are eagerly evaluated; if you are passing the result of a function call, it is recommended to
     * use {@link Option#unwrapOrElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
     ...
     */
    default @NotNull T unwrapOrElse(@NotNull @NonNull Supplier<? extends @NotNull T> supplier) {
        return switch (this) {
            case Some<T>(T value) -> value;
    ...
    /**
     * Maps an {@code Option<T>} to {@code Option<R>} by applying a function to a contained value (if Some) or returns None (if
     * None).
     *
     ...
     * @return a new {@code Option<R>} with the converted value
     */
    default <R, X extends Exception> @NotNull Option<? extends R> safeMap(@NotNull @NonNull ThrowingFunction<? super @NotNull T, ? extends @NotNull R, ? extends @NotNull X> f) throws
    ...
    /**
     * Calls the provided {@code Consumer<T>} with the contained value (if {@code Some<T>}) and returns itself. This allows for
     * chaining multiple consumers that all need the value e.g.:
     * <pre>
     ...
     * {@code Some}).
     * <p>
     * Arguments passed to {@link Option#mapOr(java.util.function.Function, Object)} are eagerly evaluated; if you are passing the
     ...
     */
    default <R> @NotNull R mapOr(@NotNull @NonNull Function<? super @NotNull T, ? extends @NotNull R> f,
                                 @NotNull @NonNull R defaultValue) {
        return switch (this) {
            case Some<T>(T value) -> f.apply(value);
            case None<T> ignored -> defaultValue;
        };
    ...
    /**
     * Computes a default function result (if {@code None}), or applies a different function to the contained value (if
     * {@code Some}).
     *
     * @param d {@code Supplier<? extends R>} that supplies a value if {@code None}
     * @param f               function that converts the value in case of {@code Some<T>}
     * @param <R>             the target type that should be mapped to
     *
    ...
    default <R> @NotNull R mapOrElse(@NotNull @NonNull Function<? super @NotNull T, ? extends @NotNull R> f,
                                     @NotNull @NonNull Supplier<? extends @NotNull R> d) {
        return switch (this) {
            case Some<T>(T value) -> f.apply(value);
            case None<T> ignored -> d.get();
        };
    }
     ...
     * {@code Err(err)}.
     * <p>
     * Arguments passed to {@link Option#okOr(Object)} are eagerly evaluated; if you are passing the result of a function call, it
     * is recommended to use {@link Option#okOrElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
    ...
    /**
     * Transforms the {@code Option<T>} into a {@code Condition<T>}, mapping {@code Some(v)} to {@code Holds(v, p)} and
     * {@code None} to {@code HoldsNot}.
     *
     * @param bound condition that should hold on the contained value
     *
     * @return a {@code Condition<T>} representing the {@code Option<T>} in form of a {@code Condition<T>}
     */
    default @NotNull Condition<T> alwaysMaintain(@NotNull @NonNull Predicate<? super @NotNull T> bound) {
        return Condition.fromOption(this, bound);
    }
     ...
     * {@link Option#andThen(java.util.function.Function)}, which is lazily evaluated.
     *
    ...
    /**
     * Returns {@code None} if the {@code Option<T>} is {@code None}, otherwise calls the {@code Function<T,Option<U>>} with the
     * wrapped value and returns the result.
     * <p>
     * Some languages call this operation flatmap.
     *
     * @param f   another optional that should be returned when this is {@code Some<T>}
     * @param <U> type of the other optional
     *
     * @return either the result of {@code other(some.value)} or {@code None} when this is {@code None}
     */
    default <U> @NotNull Option<? extends U> andThen(@NotNull @NonNull Function<? super @NotNull T, Option<? extends @NotNull U>> f) {
        return switch (this) {
            case Some<T>(T value) -> f.apply(value);
            case None<T> ignored -> none();
        };
    ...
    /**
     * Returns {@code None} if the {@code Option<T>} is {@code None}, otherwise calls {@code Predicate<T>} with the wrapped value
     * and returns:
     * <ul>
    ...
    default @NotNull Option<T> filter(@NotNull @NonNull Predicate<? super @NotNull T> predicate) {
        if (this instanceof Some<T>(T value) && (predicate.test(value))) {
            return some(value);
        }
        return none();
    }
     ...
     * {@link Option#orElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
    ...
    /**
     * Returns the {@code Option<T>} if it contains a value, otherwise returns {@code other} as an {@code Option<T>}. This means
     * if other is {@code null} the returned option is {@code None<T>}.
     * <p>
     * Arguments passed to or are eagerly evaluated; if you are passing the result of a function call, it is recommended to use
     * {@link Option#orNullableElse(java.util.function.Supplier)}, which is lazily evaluated.
     *
     * @param other the other {@code Option<T>} that should be returned instead if {@code this} is {@code None<T>}
     *
     * @return an {@code Option<T>} that is either {@code this} or the {@code other} wrapped in an {@code Option<T>}
     */
    @Contract(value = "_ -> new", pure = true)
    default @Nullable Option<T> orNullable(@Nullable T other) {
        return switch (this) {
            case Some<T>(T value) -> some(value);
     ...
     *
     * @param other a {@code Supplier} that provides the {@code other} {@code Option<T>} in case that {@code this} is
     *              {@code None<T>}
     *
     * @return the {@code Option<T>} if it contains a value, otherwise calls the {@code Supplier} and returns the result
     */
    default @NotNull Option<? extends T> orElse(@NotNull @NonNull Supplier<@NotNull Option<? extends @NotNull T>> other) {
        return switch (this) {
            case Some<T>(T value) -> some(value);
    ...
    /**
     * Returns the option if it contains a value, otherwise calls f and returns the result wrapped into an option.
     *
     * @param other a {@code Supplier} that provides the {@code other} {@code Option<T>} in case that {@code this} is
     *              {@code None<T>}
     *
     * @return the {@code Option<T>} if it contains a value, otherwise calls the {@code Supplier} and returns the result in an
     * {@code Option<T>}
     */
    @Contract("_ -> new")
    default @NotNull Option<T> orNullableElse(@NotNull @NonNull Supplier<? extends @Nullable T> other) {
        return switch (this) {
            case Some<T>(T value) -> some(value);
    ...
    /**
     * Returns {@code Some<T>} if exactly one of {@code this}, {@code other} is {@code Some<T>}, otherwise returns
     * {@code None<T>}.
     *
    ...
    @Contract(value = "_ -> new", pure = true)
    static <T> @NotNull Option<T> optionFrom(
